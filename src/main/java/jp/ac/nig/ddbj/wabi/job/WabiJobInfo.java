...
public abstract class WabiJobInfo {
	...
	/** qsub, qstat, qacctを実行する前に読み込むUGE設定用シェルスクリプトのパス */
//	public static String ugeSetting = Conf.ugeSetting;
	...
	abstract public String[] readSlurmJobId() throws IOException;
	...
	public ArrayList<String> qstatStatus(Bash bash)  {
		...
		try {
			...
			//at025のsingularityインスタンスで実行しているwabiからqstatする場合
//			BashResult res = bash.system("source /home/geadmin2/UGES/uges/common/settings.sh; qstat -j " + jobId);

//			BashResult res = bash.system("source " + ugeSetting + "; qstat -j " + jobId);
			BashResult res = bash.system("qstat -j " + jobId);
	...
	public ArrayList<String> qacctStatus(Bash bash) {
		...
		try {
			...
			// at025のsingularityインスタンスで実行しているwabiからqacctする場合
			//BashResult res = bash.system("source /home/geadmin2/UGES/uges/common/settings.sh; qacct -d 7 -j " + jobId);
			
//			BashResult res = bash.system("source " + ugeSetting + "; qacct -d 7 -j " + jobId);
			BashResult res = bash.system("qacct -d 7 -j " + jobId);
	...
	public String qsub(String jobName, String scriptName) throws IOException {
		...
		//at025のsingularityインスタンスで実行しているwabiからqsubする場合
...
//		qsub = "source " + ugeSetting + "; qsub " + Conf.qsubOptions + " " + jobName + " -S /bin/bash " + scriptName;
		qsub = "qsub " + jobName + " -S /bin/bash " + scriptName;
	...
	abstract public String[] getSbatchStdoutFilenames();

	abstract public String[] getSbatchStderrFilenames();
